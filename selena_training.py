...
init_learning_rate = 0.01 # 0.1 for purchase and texas, 0.01 for alexnet
momentum = 0.9
decay = 1e-5
#schedule = [80,100]
schedule = [100]
...
fpr_threshold = 0.005
shadow_model_number = 1 # 40
if (dataset_name == 'purchase' or dataset_name == 'texas' or dataset_name == 'location'):
...
def eval_on_ban_list(model, dataset):
	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
	vulnerable_index = np.load(f'./{dataset_name}_top500_vul_index.npy')
	...
	vul_dataloader = torch.utils.data.DataLoader(vul_dataset, batch_size=target_batch_size,
...
def selena_training():
	...
	for i in tqdm(range(total_model_number)):
		...
		#print (f" model {i}, training data number {len(this_model_train_label)}")
		class_weights = np.ones((len(np.unique(this_model_train_label)))) * training_set_size / (	len(np.unique(this_model_train_label)) * (np.bincount(this_model_train_label, minlength=len(np.unique(target_dataset.train_label))) + 10))
		...
		with torch.no_grad():
			this_model.eval()
			for images, labels, _ in this_model_train_loader:
	...
	for k, v in soft_label.items():
		...
		#print (torch.max(this_avg),this_avg)
	...
	this_model_train_eval_dataset = part_pytorch_dataset(this_model_train_data, current_round_train_label, train=True, transform=transform_train,
													target_transform=target_transform, float_target=True) ## ? transform_test
	this_model_test_dataset = part_pytorch_dataset(this_model_test_data, this_model_test_label, train=False, transform=transform_test,
												   target_transform=target_transform)
	this_model_train_loader = torch.utils.data.DataLoader(this_model_train_dataset, batch_size=target_batch_size,
														  shuffle=True, num_workers=1)
	...
	### todo: save some info for showing why purchase cannot be trained
	#name_str = './purchase_selena_debug.npz'
	#np.savez(name_str,avg_soft_label,all_train_loss,all_train_pred,current_round_train_label)
	...
	vul_acc = eval_on_ban_list(final_model,target_dataset)
	### this label is wrong here. we need to sort the label based on training partition and validation partition..
	### todo: fix the bug above
	return final_model,np.concatenate((all_train_pred,all_validation_pred),axis=0), training_partition, validation_partition, target_dataset.train_label,vul_acc
...
for shadow_idx in range(shadow_model_number):
	this_shadow_model,this_all_pred, this_all_train_index, this_all_validation_index, this_all_class_label,vul_acc = selena_training()
	...
	all_vul_acc.append(vul_acc)
	
	from nasr_whitebox_attack import nasr_whitebox_attack
	
	target_model_optim =  torch.optim.SGD(this_shadow_model.parameters(), lr=0.01, momentum=0.9, weight_decay=1e-5)
	
	#nasr_whitebox_attack = nasr_whitebox_attack(num_classes)
	#nasr_whitebox_attack.run_attack(target_model=this_shadow_model, target_model_optim=target_model_optim, target_dataset=target_dataset, training_partition=this_all_train_index,
	#								train_epoch=20, fpr_threshold=0.001)
	#exit(0)
	
	label_only_attack(this_shadow_model, target_dataset, training_index=this_all_train_index, validation_index=this_all_validation_index)
...
print (f"all vul set acc {np.average(np.array(all_vul_acc))}")
...
np.savez(all_name, all_pred, all_train_index, all_validation_index, all_class_label,fpr_threshold=fpr_threshold)
print (all_name)

print (f"FPR THRESHOLD :{fpr_threshold}")
auc, plr = get_blackbox_auc_class_nn(all_pred, all_train_index, all_validation_index, all_class_label=all_class_label,fpr_threshold=fpr_threshold)
get_blackbox_auc_lira(all_pred, all_train_index, all_validation_index, all_class_label,fpr_threshold_list=[fpr_threshold])
